...
<hr>
   ...
   <div class="dropdown_box" id="conv2" >
      ...
      <c:otherwise>
		 ...
		 <div class="dropdown_box" id="conv" >
			<ul class="nav" id="top_with_login_ul">	
...
<tr>
<td colspan="3">
<h3><b>에어아이콘과 함께 전국을 여행해보세요</b></h3>   
</td>
</tr>
<tr>
   <td>
...
<tr>
   ...
   <td>
      <a href="../detail/detail_acom?acom_no=${dto.acom_no} "><img src="../Download?fileName=${dto.save_file}" style=" padding-right:20px;   width:370px; height:250px; "></a><br/>
   
      <a href="../detail/detail_acom?acom_no=${dto.acom_no} " style="color:black; margin-top:10px;"> <font size="4"><b>${dto.intro_title}</b></font></a>
      <c:choose>
      <c:when test="${user_id != null}"><a href="bookmark?acom_no=${dto.acom_no}" style="color:#f2c921">★</a> </c:when></c:choose>
      <br/>
   <font color="#5c5c5c" size="3"> <fmt:formatNumber value="${dto.price}" pattern="\#,###"/> /박</font><br/>
     <font color="#4ebd7d" size="3"><b>${dto.rate}점</b> </font><br/>
      ...
      <td colspan="5">
        ...
        <ul class="pagination justify-content-center">
          ...
          <li class="page-item<c:if test="${startBlock==1}"> disabled</c:if>">
            <a class="page-link" href="searchviewnew?pg=${startBlock - 1}&new=notnew" tabindex="-1" aria-disabled="true">Previous</a>
          </li>
          
         <c:forEach var="p" begin="${startBlock}" end="${endBlock}">
             <li class="page-item <c:if test="${p == pg}"> active</c:if>">
                <a <c:if test="${p != pg}"> href="searchviewnew?pg=${p}&new=notnew" </c:if> class="page-link">${p}</a>
                   <c:if test="${p == pg}"><span class="sr-only">(current)</span></c:if>
             </li>
         </c:forEach>
          
          <li class="page-item<c:if test="${endBlock == totalPage}"> disabled</c:if>">
            <a class="page-link" href="searchviewnew?pg=${endBlock + 1}&new=notnew">Next</a>
          </li>
        </ul>
        </nav>
      </td>
