...
<tr>
   ...
   <td >
      ...
      <a href="../detail/detail_acom?acom_no=${dto.acom_no} " style="color:black;"> <font size="4"><b>${dto.intro_title}</b></font></a>
      <c:choose>
      ...
      <td colspan="5">
        ...
        <ul class="pagination justify-content-center">
          ...
          <li class="page-item<c:if test="${startBlock==1}"> disabled</c:if>">
            <a class="page-link" href="searchview?pg=${startBlock - 1}" tabindex="-1" aria-disabled="true">Previous</a>
          </li>
          
         <c:forEach var="p" begin="${startBlock}" end="${endBlock}">
             <li class="page-item <c:if test="${p == pg}"> active</c:if>">
                <a <c:if test="${p != pg}"> href="searchview?pg=${p}" </c:if> class="page-link">${p}</a>
                   <c:if test="${p == pg}"><span class="sr-only">(current)</span></c:if>
             </li>
         </c:forEach>
          
          <li class="page-item<c:if test="${endBlock == totalPage}"> disabled</c:if>">
            <a class="page-link" href="searchview?pg=${endBlock + 1}">Next</a>
          </li>
        </ul>
        </nav>
      </td>
