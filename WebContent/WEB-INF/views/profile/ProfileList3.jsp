				...
				<tbody>
					...
					<a href="../detail/detail_acom?acom_no=${list.acom_no}" style="color:#f78181">${list.title}</a>
						...
						<fmt:formatNumber value="${list.price}" pattern="\#,###"/>/박
							...
							<td>
							<a href="bookmark_delete?bookmark_no=${list.bookmark_no}">삭제</a>
						</td>
					</tr>
				</c:forEach>
		...
		<td colspan="6" align="center">
		  ...
		  <ul class="pagination justify-content-center">
		    ...
		    <li class="page-item<c:if test="${startBlock==1}"> disabled</c:if>">
		      <a class="page-link" href="profilelist3?pg=${startBlock - 1}" tabindex="-1" aria-disabled="true">Previous</a>
		    </li>
		    
			<c:forEach var="p" begin="${startBlock}" end="${endBlock}">
			    <li class="page-item <c:if test="${p == pg}"> active</c:if>">
			    	<a <c:if test="${p != pg}"> href="profilelist3?pg=${p}" </c:if> class="page-link">${p}</a>
			    	   <c:if test="${p == pg}"><span class="sr-only">(current)</span></c:if>
			    </li>
			</c:forEach>
		    
		    <li class="page-item<c:if test="${endBlock == totalPage}"> disabled</c:if>">
		      <a class="page-link" href="profilelist3?pg=${endBlock + 1}">Next</a>
		    </li>
		  </ul>
		  </nav>
		</td>
