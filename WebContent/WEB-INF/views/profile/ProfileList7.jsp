	...
	<div class="row"> <!-- row -->
		<div class="col-md-9">
			...
			<tr align="center">
				...
				<tbody>
			<c:forEach items="${acom_resrv_host}" var="list"  >
					<tr align="center">
						<td>
						${list.acom_reserv_no}	
						</td>
						<td>
						<font color="#4ebd7d"><b>${list.resrv_check}</b></font>
						</td>
						<td>
						...
						<td>
							${list.reserv_date_start}~${list.reserv_date_end}
						</td>
		...
		<td colspan="9" align="center">
		  ...
		  <ul class="pagination justify-content-center">
		    ...
		    <li class="page-item<c:if test="${startBlock==1}"> disabled</c:if>">
		      <a class="page-link" href="profilelist7?pg=${startBlock - 1}" tabindex="-1" aria-disabled="true">Previous</a>
		    </li>
		    
			<c:forEach var="p" begin="${startBlock}" end="${endBlock}">
			    <li class="page-item <c:if test="${p == pg}"> active</c:if>">
			    	<a <c:if test="${p != pg}"> href="profilelist7?pg=${p}" </c:if> class="page-link">${p}</a>
			    	   <c:if test="${p == pg}"><span class="sr-only">(current)</span></c:if>
			    </li>
			</c:forEach>
		    
		    <li class="page-item<c:if test="${endBlock == totalPage}"> disabled</c:if>">
		      <a class="page-link" href="profilelist7?pg=${endBlock + 1}">Next</a>
		    </li>
		  </ul>
		  </nav>
		</td>
