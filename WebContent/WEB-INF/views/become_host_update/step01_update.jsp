...
<head>
	...
	<title>호스트 등록 수정중입니다 1/3·AirIcon</title>
...
<body>
	...
	<!-- main box -->
	<form name="myform" method="post" action="acom_update_action">
	...
	<div class="main_box">
		...
		<div class="ask_01">
			...
			<select class="form-control" name="type">
    			<option selected disabled required hidden="true">하나를 선택하세요</option>
   				<option <c:if test="${acom_infoUpdateDTO.type eq '아파트'}">selected</c:if> >아파트</option>
    			<option <c:if test="${acom_infoUpdateDTO.type eq '주택'}">selected</c:if> >주택</option>
    			<option <c:if test="${acom_infoUpdateDTO.type eq '빌라'}">selected</c:if> >빌라</option>
    			<option <c:if test="${acom_infoUpdateDTO.type eq '독채'}">selected</c:if> >독채</option>
    			<option <c:if test="${acom_infoUpdateDTO.type eq '숙박 업소'}">selected</c:if> >숙박 업소</option>
    			<option <c:if test="${acom_infoUpdateDTO.type eq '독특한 숙소'}">selected</c:if> >독특한 숙소</option>    			
 			</select>			
		</div>
		...
		<div class="ask_02">
			<h4 class="ask_font"><li>숙소에 얼마나 많은 인원이 숙박할 수 있나요?</li></h4>
 			...
 			<input type="text" class="form-control" name="maxperson" placeholder="숙박 가능 인원"  value="${acom_infoUpdateDTO.maxperson}" maxlength="4"  onKeyDown="OnlyNumber()" required >
		</div>
		
		<div class="ask_03">
			<h4 class="ask_font"><li>숙소의 위치를 알려주세요.</li></h4>
			
			<input type="text" class="form-control" name="address" placeholder="${acom_infoUpdateDTO.address}" required
			disabled="disabled">
		</div>
		...
		<div class="ask_04">
			...
			<c:if test="${acom_infoUpdateDTO.price != 0}">value="${acom_infoUpdateDTO.price}"</c:if> >
		</div>
